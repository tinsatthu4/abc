...
class elFinder {
	...
	protected $_options = array (
			...
			'rootAlias' => 'Home', // display this instead of root directory
			                          // name
			...
			'mimeDetect' => 'auto', // files mimetypes detection method (finfo,
			                          // mime_content_type, linux (file -ib), bsd (file
			                          // -Ib), internal (by extensions))
			'uploadAllow' => array (), // mimetypes which allowed to upload
			'uploadDeny' => array (), // mimetypes which not allowed to upload
			'uploadOrder' => 'deny,allow', // order to proccess uploadAllow and
			                                // uploadAllow options
			'imgLib' => 'auto', // image manipulation library (imagick,
			                          // mogrify, gd)
			'tmbDir' => '.tmb', // directory name for image thumbnails. Set
			                          // to "" to avoid thumbnails generation
			'tmbCleanProb' => 1, // how frequiently clean thumbnails dir (0 -
			                     // never, 200 - every init request)
			'tmbAtOnce' => 5, // number of thumbnails to generate per request
			'tmbSize' => 48, // images thumbnails size (px)
			'tmbCrop' => true, // crop thumbnails (true - crop, false - scale
			                        // image to fit thumbnail size)
			...
			'archiveMimes' => array (), // allowed archive's mimetypes to create.
			                           // Leave empty for all available types.
			'archivers' => array ()  // info about archivers to use. See example below.
	...
	public function __construct($options = array()) {
		...
		if ($this->_options ['tmbDir']) {
			if (! in_array ( $this->_options ['imgLib'], array (
					'imagick',
					'mogrify',
					'gd' 
			) )) {
	...
	/**
	 * Check new file name for invalid simbols.
		...
		if (false != ($ls = scandir ( $path ))) {
			for($i = 0; $i < count ( $ls ); $i ++) {
				...
				if (is_link ( $p )) {
					...
					// $this->_result['debug']['findDir_'.$p] = 'link to
				// '.$link;
				}
				if ($this->_isAccepted ( $ls [$i] ) && is_dir ( $p ) && (! is_link ( $p ))) {
	...
	protected function _tmb($img, $tmb) {
		...
		if ($this->_options ['tmbCrop'] == false) {
			...
			/*
			 * Keeping original dimensions if image fitting into thumbnail
			 * without scale
			 */
		...
		switch ($this->_options ['imgLib']) {
			...
			case 'mogrify' :
				if (@copy ( $img, $tmb )) {
					...
					// exec('mogrify -crop '.$size.'x'.$size.'+'.$x.'+'.$y.'
					// -scale '.$tmbSize.'x'.$tmbSize.'! '.escapeshellarg($tmb),
					// $o, $c);
	...
	/**
	 * Return true if file name is not .
	 * or ..
	 * If file name begins with . return value according to
	 * $this->_options['dotFiles']
	 *
	 * @param string $file
	 *        	file name
	 * @return bool
	...
	/**
	 * Return normalized path, this works the same as os.path.normpath() in
	 * Python
	 *
	 * @param string $path
	 *        	path
