...
class Unzip {
	...
	private function _load_file_list_by_eof(&$fh, $stop_on_file = FALSE) {
		...
		for($x = 0; $x < 1024; $x ++) {
			...
			if ($signature == $this->central_signature_end) {
				...
				$eodir ['disk_number_this'] = unpack ( "v", fread ( $fh, 2 ) ); // number
				                                                         // of this disk
				$eodir ['disk_number'] = unpack ( "v", fread ( $fh, 2 ) ); // number of the
				                                                    // disk with the start
				                                                    // of the central
				                                                    // directory
				$eodir ['total_entries_this'] = unpack ( "v", fread ( $fh, 2 ) ); // total
				                                                           // number of
				                                                           // entries in the
				                                                           // central dir on
				                                                           // this disk
				$eodir ['total_entries'] = unpack ( "v", fread ( $fh, 2 ) ); // total
				                                                      // number of entries
				                                                      // in
				$eodir ['size_of_cd'] = unpack ( "V", fread ( $fh, 4 ) ); // size of the
				                                                   // central directory
				$eodir ['offset_start_cd'] = unpack ( "V", fread ( $fh, 4 ) ); // offset of
				                                                        // start of central
				                                                        // directory with
				                                                        // respect to the
				                                                        // starting disk
				                                                        // number
				$zip_comment_lenght = unpack ( "v", fread ( $fh, 2 ) ); // zipfile comment
				                                                  // length
				$eodir ['zipfile_comment'] = $zip_comment_lenght [1] ? fread ( $fh, $zip_comment_lenght [1] ) : ''; // zipfile
				                                                                                              // comment
				...
				while ( $signature == $this->dir_signature ) {
					$dir ['version_madeby'] = unpack ( "v", fread ( $fh, 2 ) ); // version
					                                                     // made by
					$dir ['version_needed'] = unpack ( "v", fread ( $fh, 2 ) ); // version
					                                                     // needed to extract
					$dir ['general_bit_flag'] = unpack ( "v", fread ( $fh, 2 ) ); // general
					                                                       // purpose bit flag
					$dir ['compression_method'] = unpack ( "v", fread ( $fh, 2 ) ); // compression
					                                                         // method
					$dir ['lastmod_time'] = unpack ( "v", fread ( $fh, 2 ) ); // last mod
					                                                   // file time
					$dir ['lastmod_date'] = unpack ( "v", fread ( $fh, 2 ) ); // last mod
					                                                   // file date
					$dir ['crc-32'] = fread ( $fh, 4 ); // crc-32
					$dir ['compressed_size'] = unpack ( "V", fread ( $fh, 4 ) ); // compressed
					                                                      // size
					$dir ['uncompressed_size'] = unpack ( "V", fread ( $fh, 4 ) ); // uncompressed
					                                                        // size
					$zip_file_length = unpack ( "v", fread ( $fh, 2 ) ); // filename length
					$extra_field_length = unpack ( "v", fread ( $fh, 2 ) ); // extra field
					                                                  // length
					$fileCommentLength = unpack ( "v", fread ( $fh, 2 ) ); // file comment
					                                                 // length
					$dir ['disk_number_start'] = unpack ( "v", fread ( $fh, 2 ) ); // disk
					                                                        // number start
					$dir ['internal_attributes'] = unpack ( "v", fread ( $fh, 2 ) ); // internal
					                                                          // file
					                                                          // attributes-byte1
					$dir ['external_attributes1'] = unpack ( "v", fread ( $fh, 2 ) ); // external
					                                                           // file
					                                                           // attributes-byte2
					$dir ['external_attributes2'] = unpack ( "v", fread ( $fh, 2 ) ); // external
					                                                           // file
					                                                           // attributes
					$dir ['relative_offset'] = unpack ( "V", fread ( $fh, 4 ) ); // relative
					                                                      // offset of local
					                                                      // header
					$dir ['file_name'] = fread ( $fh, $zip_file_length [1] ); // filename
					$dir ['extra_field'] = $extra_field_length [1] ? fread ( $fh, $extra_field_length [1] ) : ''; // extra
					                                                                                        // field
					$dir ['file_comment'] = $fileCommentLength [1] ? fread ( $fh, $fileCommentLength [1] ) : ''; // file
					                                                                                       // comment
					                                                                                       
					// Convert the date and time, from MS-DOS format to UNIX
					                                                                                       // Timestamp
				...
				// If loaded centralDirs, then try to identify the
				// offsetPosition of the compressed data.
	...
	private function _load_files_by_signatures(&$fh, $stop_on_file = FALSE) {
		...
		for(;;) {
			...
			if (! $details) {
				...
				fseek ( $fh, 12 - 4, SEEK_CUR ); // 12: Data descriptor - 4: Signature
				                              // (that will be read again)
				$details = $this->_get_file_header ( $fh );
			}
	...
	private function _get_file_header(&$fh, $start_offset = FALSE) {
		...
		if ($signature == $this->zip_signature) {
			...
			$file ['version_needed'] = unpack ( "v", fread ( $fh, 2 ) ); // version needed
			                                                      // to extract
			$file ['general_bit_flag'] = unpack ( "v", fread ( $fh, 2 ) ); // general
			                                                        // purpose bit flag
			$file ['compression_method'] = unpack ( "v", fread ( $fh, 2 ) ); // compression
			                                                          // method
			$file ['lastmod_time'] = unpack ( "v", fread ( $fh, 2 ) ); // last mod file
			                                                    // time
			$file ['lastmod_date'] = unpack ( "v", fread ( $fh, 2 ) ); // last mod file
			                                                    // date
			$file ['crc-32'] = fread ( $fh, 4 ); // crc-32
			$file ['compressed_size'] = unpack ( "V", fread ( $fh, 4 ) ); // compressed
			                                                       // size
			$file ['uncompressed_size'] = unpack ( "V", fread ( $fh, 4 ) ); // uncompressed
			                                                         // size
			$zip_file_length = unpack ( "v", fread ( $fh, 2 ) ); // filename length
			$extra_field_length = unpack ( "v", fread ( $fh, 2 ) ); // extra field length
			$file ['file_name'] = fread ( $fh, $zip_file_length [1] ); // filename
			$file ['extra_field'] = $extra_field_length [1] ? fread ( $fh, $extra_field_length [1] ) : ''; // extra
			                                                                                         // field
